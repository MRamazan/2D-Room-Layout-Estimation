    ...
    for scene_id in scene_ids:
        ...
        if reload_scene:
            try:
                ...
                print(f"Scene {root_path, scene_id} not available","\n\n\n\n")
...
class Structured3DPlans_Perspective(Dataset):
    ...
    def __getitem__(self, idx, room_id=None, room_sample_id=None):
        ...
        import cv2
        try:
            cv2.imshow("sdafa",image) / cv2.imshow("sdafa",image_orig) / cv2.imshow("sdafa",persp_pose) / cv2.imshow("sdafa",pano_pose)
            cv2.waitKey(0)
        except:
            print("FDHSGHJKHSDKGFG//\n\n\n\n\n\n\n\n1sdfgsdfgsdfFDHGSDFHFGSDH")
