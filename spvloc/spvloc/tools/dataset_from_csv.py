...
from projection import pano2persp_from_rotation, pano2persp_get_camera_parameter
...
def process_csv(path_to_csv, path_in, path_out):
    ...
    with open(path_to_csv, "r") as csv_file:
        ...
        for row in tqdm(csv_reader, total=total_rows):
            ...
            sample_path = os.path.normpath(sample_path).replace("empty_90_20_20","full").replace("full_90_20_20","full")
            if os.path.sep == "/":
            ...
            transformed_path = sample_path.replace("perspective", "perspective")


            # Cut after "panorama\" if it exists
            index = transformed_path.find("perpective" + os.path.sep)
            if index != -1:
                transformed_path = transformed_path[: index + len("perspective" + os.path.sep)]

            # Add furniture component and file name
            transformed_path = os.path.join(transformed_path, "rgb_rawlight.png")
...
def parse_arguments():
    ...
    parser.add_argument("-d", "--path_to_csv", type=str,default="data/datasets/s3d/dataset_info.csv", help="Path to the CSV file")
    parser.add_argument("-i", "--path_in", default="../dataset",type=str, help="Input path")
    parser.add_argument("-o", "--path_out", type=str, default="/home/godling/Downloads/deneme-main/layout_pred/results",help="Output path")
