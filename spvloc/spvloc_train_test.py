...
def load_test_checkpoint(checkpoint_path, model):
    ...
    load = torch.load(checkpoint_path,map_location=torch.device('cpu'))
...
def main_start():
    ...
    args.config_file = "configs/config_eval_s3d_perspective.yaml"
    args.test_ckpt = "data/pretrained_models/ckpt_s3d_16_9.ckpt"
    ...
    config.TEST.SAVE_PLOT_OUTPUT = True
    config.TEST.SAVE_PLOT_DETAILS = True
    config.TEST.PLOT_OUTPUT = True
    config.freeze()
